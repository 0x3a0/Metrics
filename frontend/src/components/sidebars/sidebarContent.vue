...
<template>
  <div class="h-full flex flex-col">
    <div class="p-6 border-b border-gray-100 select-none">
    ...
    <nav class="flex-1 p-3 space-y-1 mt-2">
      <router-link
        ...
        :key="item.id"
        :to="{ name: item.id === 'dashboard' ? 'Dashboard' : item.id === 'account' ? 'Account' : 'Settings' }"
        :class="[
          ...
          route.name === (item.id === 'dashboard' ? 'Dashboard' : item.id === 'account' ? 'Account' : 'Settings')
